...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        # Step 1 — flag each chunk as relevant if it covers >= threshold of expected.
        relevant_flags = [
        ...
        # Steps 2-3 — rank-aware Average Precision (AP@K).
        ap, hits = 0.0, 0
        for k, is_relevant in enumerate(relevant_flags, start=1):
            if is_relevant:
                hits += 1
                ap += hits / k  # Precision@k counted only at relevant positions
        return max(0.0, min(1.0, ap / num_relevant))
    ...
    # -----------------------------------------------------------------------
    # Custom metric (Bonus) — conciseness / anti-verbosity
    # -----------------------------------------------------------------------
    def evaluate_conciseness(self, answer: str, expected: str) -> float:
        """Custom metric: penalize verbose answers (mitigates verbosity bias).

        1.0 if the answer is at most ~2x the length of the expected answer,
        decaying toward 0.0 as the answer grows longer. Returns 1.0 when either
        side is empty (nothing to compare).
        """
        a = len(_tokenize(answer))
        e = len(_tokenize(expected))
        if a == 0 or e == 0:
            return 1.0
        ratio = a / e
        if ratio <= 2.0:
...
# ---------------------------------------------------------------------------
# Bonus — second "framework" for cross-framework comparison (Exercise 3.4)
# ---------------------------------------------------------------------------
class JaccardEvaluator:
    """An alternative evaluator using Jaccard similarity |A ∩ B| / |A ∪ B|.

    Same three answer-side metrics as RAGASEvaluator but with the UNION as the
    denominator, so it also penalizes *extra* tokens (verbosity / off-topic
    padding). It is therefore systematically STRICTER than the overlap-by-
    denominator RAGASEvaluator — useful to compare two frameworks on one dataset.
    """
    ...
    def evaluate_faithfulness(self, answer: str, context: str) -> float:
        ...
        if not answer_tokens:
            return 1.0
        return self._jaccard(answer_tokens, _tokenize(context))

    def evaluate_relevance(self, answer: str, question: str) -> float:
        question_tokens = _tokenize(question)
        if not question_tokens:
            return 1.0
        return self._jaccard(_tokenize(answer), question_tokens)

    def evaluate_completeness(self, answer: str, expected: str) -> float:
        expected_tokens = _tokenize(expected)
        if not expected_tokens:
            return 1.0
        return self._jaccard(_tokenize(answer), expected_tokens)
...
class LLMJudge:
    ...
    def score_response(
        ...
        """
        # 1. Build a judge prompt with the question, answer, and rubric.
        ...
        prompt = (
            ...
            f"Answer: {answer}\n"
            f"Rubric:\n{rubric_lines}\n"
            'Respond ONLY with JSON, e.g. {"accuracy": 0.8, "clarity": 0.7}.'
        )

        # 2. Call the judge LLM.
        raw = self.judge_llm_fn(prompt)

        # 3. Parse scores; fall back to 0.5 per criterion if unparseable.
        try:
            parsed = json.loads(raw)
            scores = {name: float(parsed.get(name, 0.5)) for name in rubric}
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        all_scores = [s for entry in scores_batch for s in entry.get("scores", {}).values()]
        avg = sum(all_scores) / len(all_scores) if all_scores else 0.0

        # Positional bias: the first response consistently scores higher than the rest.
        positional_bias = False
        if len(scores_batch) >= 2:
            first = mean_of(scores_batch[0])
            rest = [mean_of(e) for e in scores_batch[1:]]
            rest_avg = sum(rest) / len(rest) if rest else 0.0
            positional_bias = first > rest_avg
...
class BenchmarkRunner:
    ...
    def run(
        ...
        for pair in qa_pairs:
            ...
            # Keep the original pair (carries metadata) on the result.
            result.qa_pair = pair
            # If retrieved chunks are provided, also score the retriever.
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        # If two or more metrics are critically low, the whole pipeline is suspect.
        if sum(1 for v in scores.values() if v < 0.3) >= 2:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for i, failure in enumerate(failures, start=1):
            ...
            fix = suggestions[i - 1] if i - 1 < len(suggestions) else "Review and triage"
    ...
    def generate_improvement_suggestions(
        ...
        by_type = {
            "hallucination": "Implement a faithfulness/hallucination checker to filter unsupported claims; add a cite-or-refuse guardrail.",
            "irrelevant": "Improve prompt clarity and add intent/routing detection so answers address the question.",
            "incomplete": "Increase retrieval top-k / context window and add few-shot examples showing complete answers.",
            "off_topic": "Strengthen intent detection and scope-guarding to keep answers on topic.",
            "refusal": "Loosen over-strict guardrails so valid questions get answered.",
        }
        suggestions: list[str] = [
            by_type[ftype] for ftype in self.categorize_failures(failures) if ftype in by_type
        ]

        # Ensure at least 3 actionable suggestions with generic fallbacks.
        generics = [
            "Expand the golden dataset with more edge cases and re-run the benchmark.",
            "Add citation-accuracy checks to improve answer trustworthiness.",
            "Tune chunk size/overlap to reduce evidence fragmentation.",
        ]
        for g in generics:
